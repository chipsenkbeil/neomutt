...
struct PatternHead *mutt_pattern_comp(/* const */ char *s, int flags, struct Buffer *err)
{
  ...
  if (SLIST_NEXT(SLIST_FIRST(curlist), entries))
  {
    struct Pattern *pat2;
    tmp = mutt_pattern_new_node();
    pat2 = SLIST_FIRST(tmp);
    pat2->op = or ? MUTT_PAT_OR : MUTT_PAT_AND;
    pat2->child = curlist;
    curlist = tmp;
  }
                      ...
                      struct Mailbox *m, struct Email *e, struct PatternCache *cache)
{
  ...
  switch (first->op)
  {
    ...
    case MUTT_PAT_CRYPT_SIGN:
      if (!WithCrypto)
        break;
      return first->not^((e->security & SEC_SIGN) ? 1 : 0);
    case MUTT_PAT_CRYPT_VERIFIED:
      if (!WithCrypto)
        break;
      return first->not^((e->security & SEC_GOODSIGN) ? 1 : 0);
    case MUTT_PAT_CRYPT_ENCRYPT:
      if (!WithCrypto)
        break;
      return first->not^((e->security & SEC_ENCRYPT) ? 1 : 0);
    ...
    case MUTT_PAT_DRIVER_TAGS:
    {
      ...
      bool rc = (first->not^(tags &&patmatch(first, tags) == 0));
      FREE(&tags);
      return rc;
    }
